...
export default new Vuex.Store({
  ...
  mutations: {
    setcupcakeFarm(state, payload) {
      state.cupcakeFarm = payload
    },

    setcupcakeToken(state, payload) {
      state.cupcakeToken = payload
    },

    setcupcakeTokenSupply(state, payload) {
      state.cupcakeSupply = payload
    },

    setcupcakeFarmSupply(state, payload) {
      state.cupcakeFarmSupply = payload
    },
    ...
    setcupcakeBalance(state, payload) {
      state.cupcakeBalance = payload
    },
    ...
    setcupcakeFarmBalance(state, payload) {
      state.cupcakeFarmBalance = payload
    },
    ...
    setRewards(state, payload) {
      if (payload['BURGER-bcupcake']) {
        state.currentBurgerRewards = payload['BURGER-bcupcake'].current;
        state.totalBurgerRewards = payload['BURGER-bcupcake'].total;
        state.apyBurger = payload['BURGER-bcupcake'].apy;
      }
      if (payload['WBNB-bcupcake']) {
        state.currentBNBRewards = payload['WBNB-bcupcake'].current;
        state.totalBNBRewards = payload['WBNB-bcupcake'].total;
        state.apyBNB = payload['WBNB-bcupcake'].apy;
      }
    },

    resetUser(state) {
      state.metamaskAccount = null;
      state.cupcakeBalance = 0;
      state.isApproved = false;
      state.earned = 0;
      state.availableToDeposit = 0;
      state.cupcakeFarmBalance = 0;
  ...
  actions: {
    ...
    async getRewards({commit, state}) {
      if (state.metamaskAccount) {
        const res = await apiRequest.get(`https://api.cupcake.finance/address/${state.metamaskAccount}/rewardBalance:get`)
        if (res.data) {
    ...
    async connectMetamask({state, commit, dispatch}) {
      // getting general cupcake data
      dispatch('getcupcakeData');

      if (!state.metamaskConnector) {
        commit('setMetamaskConnector', new MetamaskConnector())
        // creating contracts
        commit('setcupcakeFarm', createContract('farm'));
        commit('setcupcakeToken', createContract('rewards'));
    ...
    async getUserData({commit, state}) {
      if (state.metamaskAccount) {
        if (!state.cupcakeToken) {
          Vue.prototype.$toasted.error(`cupcake Token: the contract is not connected`, { duration: 5000 });
          return 
        }
        if (!state.cupcakeFarm) {
          Vue.prototype.$toasted.error(`cupcake Farm: the contract is not connected`, { duration: 5000 });
          return
        }
        if (!state.cupcakeFarm) {
        ...
        commit('setcupcakeBalance', await state.cupcakeToken.methods.balanceOf(state.metamaskAccount).call());
        commit('setEarned', await state.cupcakeFarm.methods.earned(state.metamaskAccount).call());
        ...
        commit('setcupcakeFarmBalance', await state.cupcakeFarm.methods.balanceOf(state.metamaskAccount).call())
      }
    },

    async getcupcakeData({ commit, state }) {
      if (state.cupcakeToken) {
        try {
          commit('setcupcakeTokenSupply', await state.cupcakeToken.methods.totalSupply().call());
        } catch (err) {
          if (checkNetwork()) {
            console.error(`cupcake Token: ${err.message}`);
            Vue.prototype.$toasted.error(`cupcake Token: ${err.message}`, { duration: 5000 });
          }
        }
      }
      if (state.cupcakeFarm) {
        try {
          commit('setRewardPerToken', await state.cupcakeFarm.methods.rewardPerToken().call());
          commit('setRewardPerBlock', await state.cupcakeFarm.methods.rewardRate().call());
          commit('setFinishPeriod', await state.cupcakeFarm.methods.periodFinish().call());
          commit('setRewardsDuration', await state.cupcakeFarm.methods.rewardsDuration().call());
          commit('setcupcakeFarmSupply', await state.cupcakeFarm.methods.totalSupply().call());
        } catch (err) {
          if (checkNetwork()) {
            console.error(`cupcake Farm: ${err.message}`);
            Vue.prototype.$toasted.error(`cupcake Farm: ${err.message}`, { duration: 5000 });
          }
        }
      }
    ...
    async harvest({ state, dispatch }) {
      ...
      if (state.cupcakeFarm) {
        ...
        const res = await state.cupcakeFarm.methods.getReward().send({from: state.metamaskAccount})
    ...
    async withdraw({ state, dispatch }, amount) {
      if (state.cupcakeFarm) {
        try {
          Vue.prototype.$toasted.success('Please wait', { duration: 0 });
          const res = await state.cupcakeFarm.methods.withdraw((new BN((amount * 1e18).toLocaleString('fullwide', {useGrouping:false}), 10))).send({from:state.metamaskAccount});
          Vue.prototype.$toasted.clear();
          if (res.status) {
            Vue.prototype.$toasted.success('Transaction confirmed!', { duration: 5000 });
            dispatch('getUserData');
  ...
  getters: {
    apy(state) {
      if (state.graph && state.stakingTotalSupply) {
        ...
        const yReward = (state.rewardPerBlock / state.cupcakeFarmSupply) * 31536000
        const cupcakePrice = token0.derivedETH
        const totalLiquidity = token0.totalLiquidity * 2 * token0.derivedETH
        const lpPrice = totalLiquidity / (state.stakingTotalSupply/1e18)
        if (lpPrice)
          // apy = (yReward * cupcakePrice) / lpPrice * 100
          return (yReward * cupcakePrice) / lpPrice * 100
      }
      return 0
    }
  },
